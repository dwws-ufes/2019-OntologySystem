...
public class RequestParticipationAppBean implements RequestParticipationApp {
	...
	private void sendEMail(Access access) {
		...
		String text = access.getEmail_text();
		String addressee = accessDAO.getCreatorAccess(access.getTarget()).getSource().getEmail();
		
		try {
			System.out.println(" -----------------: " + addressee);
			Mailer.sendEmail(addressee, "Requisição de participação",text);
		}
	...
	public List<Access> getRequests() {
		User user = SessionInformation.getInstance().getUsuarioLogado();
		return accessDAO.getRequests(user);
	}
